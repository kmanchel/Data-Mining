...
#Running the above functions as per the specified split fractions:
F = [0.01, 0.1, 0.2, 0.5, 0.6, 0.75, 0.9, 1]
bins = 10
training_accuracies = []
testing_accuracies = []
discretize_and_split(bins)
for i in F:
    training_acc, testing_acc = nbc(i)
    training_accuracies.append(training_acc)
    testing_accuracies.append(testing_acc)

#Plotting the accuracies:

#plot = pd.DataFrame(index=F)
#plot['training_acc'] = training_accuracies
#plot['testing_acc'] = testing_accuracies
#plot.head()
#plot.plot(title = "y-axis:Accuracy vs x-axis:F")


output_file("5_3.html")

p = figure(plot_width=400, plot_height=400, title="Blue: Training, Red: Test", x_axis_label='F', y_axis_label='Accuracy')

# add a line renderer
p.line(x=F, y=training_accuracies, line_width=2, line_color = 'blue')
p.line(x=F, y=testing_accuracies, line_width=2, line_color = 'red')
